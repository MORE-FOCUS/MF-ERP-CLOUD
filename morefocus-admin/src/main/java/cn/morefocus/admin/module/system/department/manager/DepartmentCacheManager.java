...
public class DepartmentCacheManager {
    ...
    public List<DepartmentTreeVO> buildTree(List<DepartmentVO> voList) {
        ...
        List<DepartmentTreeVO> treeVOList = LocalBeanUtil.copyList(rootList, DepartmentTreeVO.class);
     ...
     * 通过部门id,获取当前以及下属部门
     */
    public List<Long> selfAndChildrenIdList(Long deptId, List<DepartmentVO> voList) {
        List<Long> selfAndChildrenIdList = Lists.newArrayList();
        if (CollectionUtils.isEmpty(voList)) {
            return selfAndChildrenIdList;
        }
        selfAndChildrenIdList.add(deptId);
        List<DepartmentTreeVO> children = this.getChildren(deptId, voList);
     ...
     * 递归查询
     */
    public void selfAndChildrenRecursion(List<Long> selfAndChildrenIdList, Long deptId, List<DepartmentVO> voList) {
        List<DepartmentTreeVO> children = this.getChildren(deptId, voList);
        if (CollectionUtils.isEmpty(children)) {
            return;
        }
