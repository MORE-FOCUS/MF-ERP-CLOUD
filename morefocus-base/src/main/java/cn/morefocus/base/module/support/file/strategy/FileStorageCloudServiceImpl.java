...
public class FileStorageCloudServiceImpl implements IFileStorageService {
    ...
    @Override
    public R<FileUploadVO> upload(MultipartFile file, String path) {
        // 设置文件 key
        String originalFileName = file.getOriginalFilename();
        if (LocalStringUtils.isEmpty(originalFileName)) {
            return R.userErrorParam("上传文件名为空");
        }
    ...
    public R<String> getFileUrl(String fileKey) {
        ...
        if (!fileKey.startsWith(FileFolderTypeEnum.FOLDER_PRIVATE)) {
            // 不是私有的 都公共读
            return R.ok(cloudConfig.getUrlPrefix() + fileKey);
        }
     ...
     */
    @Override
    public R<FileDownloadVO> download(String key) {
        //获取oss对象
        ...
        try {
            ...
            return R.error(SystemErrorCode.SYSTEM_ERROR, "下载失败");
        } finally {
            try {
                // 关闭输入流
                objectContent.close();
